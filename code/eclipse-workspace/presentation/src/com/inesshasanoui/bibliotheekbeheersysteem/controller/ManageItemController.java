...
public class ManageItemController {
	...
	private void fillComboCategory(ManageItemsPanel manageItemsPanel) {
	...
	private void fillComboSearch(ManageItemsPanel manageItemsPanel) {
	...
	private void fillComboType(ManageItemsPanel manageItemsPanel) {
		for (TypeItem type : TypeItem.values()) {
			manageItemsPanel.getTypeCombo().addItem(type.toString());
		}
	}

	private Item getItemFromManageItemsPanel(ManageItemsPanel manageItemsPanel) {
		...
		CategoryItem categoryName = CategoryItem.valueOf(manageItemsPanel.getCategoryCombo().getSelectedItem().toString());
		...
		return ItemFactory.getItem(id, isbn, title, author, typeItem, summary, category, null);

	}

	private CopyItem getCopyItemFromManageCopyItemPanel(ManageCopyItemPanel manageCopyItemPanel) {
	...
	private void intializeTableDataItems() {
		JTable jTable = manageItemsPanel.getTableItems();
		DefaultTableModel tableModel = (DefaultTableModel) jTable.getModel();
		tableModel.setRowCount(0);
	}

	private void intializeTableDataCopyItems() {
		JTable jTable = manageCopyItemPanel.getTable();
		DefaultTableModel tableModel = (DefaultTableModel) jTable.getModel();
		tableModel.setRowCount(0);
	}

	private void setManageItemsPanelFromSelectedRowFromJTableItems() {
		JTable jTable = manageItemsPanel.getTableItems();
	...
	private void setManageCopyItemPanelFromItemPanel() {
	...
	private void InitializeManageItemTextField() {
	...
	private void InitializeManageCopyItemTextField() {
	...
	private void setUpTableDataCopyItem(CopyItem copyItem) {
		JTable jTable = manageCopyItemPanel.getTable();
		DefaultTableModel tableModel = (DefaultTableModel) jTable.getModel();
	...
	private void setManageCopyItemPanelFromTableCopies() {
		JTable jTable = manageCopyItemPanel.getTable();
		int selectedRow = jTable.getSelectedRow();
